...
public sealed partial class BlockingSystem : EntitySystem
{
    ...
    public bool RaiseShield(Entity<BlockingComponent> entity, EntityUid user)
    {
        ...
        if (playerTileRef != null)
        {
            ...
            {
                if (uid != user && _mobQuery.HasComponent(uid) && _physicsQuery.TryComp(uid, out var physicsComp) && physicsComp.CanCollide) // Trauma - Fix non collidable entities such as pAIs or positronic brains preventing blocking.
                {
                    TooCloseError(user);
                    return false;
                }

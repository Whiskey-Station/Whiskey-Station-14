...
public abstract partial class SharedTemperatureSystem : EntitySystem
{
    ...
    public float ConductHeat<T>(Entity<TemperatureComponent?> entity, ref T heatContainer, float deltaT, float heatTransferMod = 1f, bool ignoreHeatResistance = false) where T : IHeatContainer
    {
        ...
        {
            // <Trauma> - add data and let it be cancelled
            var ev = new BeforeHeatExchangeEvent()
            {
                Target = entity,
                OurTemp = entity.Comp.Temperature,
                OtherTemp = heatContainer.Temperature
            };
    ...
    public float ChangeHeat(Entity<TemperatureComponent?> entity, float heatAmount, bool ignoreHeatResistance = false)
    {
        ...
        if (!ignoreHeatResistance)
        {
            ...
            var ev = new BeforeHeatExchangeEvent()
            {
                Target = entity,
                OurTemp = entity.Comp.Temperature,
                OtherTemp = entity.Comp.Temperature + entity.Comp.HeatCapacity * heatAmount
            };
            RaiseLocalEvent(entity, ref ev);
            if (ev.Cancelled)
                return 0f;
    ...
    /// </summary>
    public void SetTemperature(Entity<TemperatureComponent?> entity, float temp) // Trauma - made public
    {

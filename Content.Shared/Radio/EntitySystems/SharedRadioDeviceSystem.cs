...
public abstract partial class SharedRadioDeviceSystem : EntitySystem
{
    ...
    {
        if (ent.Owner == args.RadioSource || ent.Comp.PowerRequired && !_power.IsPowered(ent.Owner)) // Trauma - check power if needed
            return;
        ...
        _chat.TrySendInGameICMessage(ent.Owner,
            // <Trauma> - dehardcode with fields, language shit
            args.OriginalChatMsg.Message,
            ent.Comp.SpeakNormally ? InGameICChatType.Speak : InGameICChatType.Whisper,
            ent.Comp.HideChat ? ChatTransmitRange.HideChat : ChatTransmitRange.GhostRangeLimit,

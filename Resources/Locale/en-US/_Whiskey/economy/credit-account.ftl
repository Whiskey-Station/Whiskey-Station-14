# SPDX-FileCopyrightText: 2026 Zequinza <felipe828218@gmail.com>
# SPDX-FileCopyrightText: 2026 Whiskey Station Contributors
#
# SPDX-License-Identifier: AGPL-3.0-or-later

credit-account-examine = Balance: [color=#4d9b31]{ $saldo }[/color] spesos.
credit-account-deposit = Deposited { $valor } spesos.
credit-account-deposit-failed = The card refuses the deposit.
credit-account-withdraw = Withdrew { $valor } spesos.
credit-account-withdraw-verb = Withdraw { $valor }
credit-account-withdraw-all = Withdraw everything ({ $valor })
cargo-sale-commission = Commission: { $valor } spesos.
credit-vendor-card-slot = ID card
credit-vendor-no-card = Insert an ID card first.
store-cartridge-program-name = General Store
store-cartridge-balance = Balance: { $saldo } spesos
store-cartridge-buy = Buy
store-cartridge-bought = Bought. { $saldo } spesos left.
store-cartridge-no-card = No ID card in this PDA.
store-cartridge-no-funds = Not enough spesos.
store-cartridge-recipient = To:
store-cartridge-recipient-self = myself
credit-account-withdraw-category = Withdraw

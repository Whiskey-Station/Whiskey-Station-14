# SPDX-FileCopyrightText: 2026 Zequinza <felipe828218@gmail.com>
# SPDX-FileCopyrightText: 2026 Whiskey Station Contributors
#
# SPDX-License-Identifier: AGPL-3.0-or-later

credit-account-examine = Saldo: [color=#4d9b31]{ $saldo }[/color] spesos.
credit-account-deposit = Depositou { $valor } spesos.
credit-account-deposit-failed = O cartão recusa o depósito.
credit-account-withdraw = Sacou { $valor } spesos.
credit-account-withdraw-verb = Sacar { $valor }
credit-account-withdraw-all = Sacar tudo ({ $valor })
cargo-sale-commission = Comissão: { $valor } spesos.
credit-vendor-card-slot = Cartão de identificação
credit-vendor-no-card = Insira um cartão primeiro.
store-cartridge-program-name = Loja Geral
store-cartridge-balance = Saldo: { $saldo } spesos
store-cartridge-buy = { $nome } ({ $preco })
store-cartridge-bought = Comprou. Sobraram { $saldo } spesos.
store-cartridge-no-card = Não tem cartão de identificação neste PDA.
store-cartridge-no-funds = Spesos insuficientes.
